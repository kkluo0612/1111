...
Receive a dile using code:
	...
	app.Commands = []*cli.Command{
		...
		},
		{
			...
			Action:      relay,
			Flags: []cli.Flag{
				&cli.StringFlag{Name: "host", Usage: "host to the relay"},
	...
	app.Action = func(c *cli.Context) error {
		allStringAreFiles := func(strs []string) bool {
		...
		if c.Args().Present() && allStringAreFiles(c.Args().Slice()) {
			fnames := []string{}
...
func getConfigFile() string {
	...
	return path.Join(configFile, "send.josn")
}
...
func receive(c *cli.Context) (err error) {
	comm.Socks5Proxy = c.String("sock5")
	crocOptions := croc.Options{
		SharedSecret:  c.String("code"),
		IsSender:      false,
		Debug:         c.Bool("debug"),
